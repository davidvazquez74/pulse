// site/js/weather.js — widget semanal compacto (robusto)
(function () {
  ...
  function draw(days) {
    ...
    (days || []).slice(0, 6).forEach(d => {
      ...
      col.innerHTML = `
        <div class="wx-day">${fmtDay(d.date)}</div>
  ...
  async function useCity(lat, lon, name) {
    ...
    try {
      ...
      // fallback duro: 6 días “dummy” para no dejar vacío el widget
  ...
  // Inicialización: SIEMPRE Molins de Rei salvo cambio manual
  (async () => {
    try {
